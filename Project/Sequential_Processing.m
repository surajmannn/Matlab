function [Sequential_Results] = Sequential_Processing(FileName, DataSize, Hours)
%% This script sequentially processes the input data file via the inputted parameters

% This allows you to open and explore the data in a *.nc file
Contents = ncinfo(FileName);
Lat = ncread(FileName, 'lat'); % load the latitude locations
Lon = ncread(FileName, 'lon'); % loadthe longitude locations
Sequential_Results = []; % create results array
...
for idx1 = 1:length(DataSize) % Loop through each data size from input argument
    DataParameter = DataSize(idx1); % current data size assigned to DataParameter
    for NumHour = 1:Hours % loop through 1 to input argument Hours
        ...
        t1 = toc;
        t2 = t1;
        for idx = 1:DataParameter % size of data to process per hour
    ...
    Sequential_Results = [Sequential_Results; DataParameter, tSeq]; % append time to results array
end % end of data size loop
end % end function